...
export async function handle({ event, resolve }) {
  ...
  const onUnprotectedRoutes = unProtectedRoutes.some((route) =>
    event.url.pathname.endsWith(route),
  );
  ...
  try {
    ...
    if (
      thrownError instanceof jwtDecode.InvalidTokenError &&
    ...
    throw error(
      500,
      "Something went wrong when trying to check your info. You are either not logged in or your token has been tampered with.",
    );
  }
  ...
  const response = await resolve(event, {
    transformPageChunk: ({ html }) =>
      html.replace(
        "%app.theme%",
        event.url.pathname.endsWith("/embed")
          ? "embed-theme"
